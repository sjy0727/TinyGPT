...
out_dir = 'out-poetry-char'  # ignored if init_from is not 'resume'
start = "\n"  # or "<|endoftext|>" or etc. Can also specify a file, use as: "FILE:prompt.txt"
num_samples = 1  # number of samples to draw
max_new_tokens = 33  # number of tokens generated in each sample
...
def load_model():
    ...
    if load_meta:
        ...
        # 将与要编码的特殊标记相对应的"<|endoftext|>"文本视为特殊标记。
...
def my_load_model():
    ...
    state_dict = torch.load('my_ckpt.ckpt', map_location=device)
    model.load_state_dict(state_dict)
    model.eval()
    model.to(device)
...
def my_infer(model, start, encode, decode, ctx, max_new_tokens):
    # encode the beginning of the prompt
    start_ids = encode(start, add_special_tokens=False)['input_ids']
    x = (torch.tensor(start_ids, dtype=torch.long, device=device)[None, ...])
...
def my_infer_iter(model, start, encode, decode, ctx, max_new_tokens):
    # encode the beginning of the prompt
    start_ids = encode(start,add_special_tokens=False)['input_ids']
    x = (torch.tensor(start_ids, dtype=torch.long, device=device)[None, ...])
    # run generation
